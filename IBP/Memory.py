...
class LSTMModel(nn.Module):
    def __init__(self, input_size=21173, context_size=100, hidden_size=100, 
        ...
        if self.cuda_flag:
            ...
            self.cell_state = torch.zeros(1, context_size).cuda()
        else:
            self.lstm_cell = nn.LSTMCell(input_size, hidden_size)
            self.hidden = (torch.zeros(1,1,self.hidden_size),
                       torch.zeros(1,1,self.hidden_size))
            self.cell_state = torch.zeros(1, context_size)
